...
class TestCaseUploadAPITest(APITestCase):
    ...
    def test_filter_file_name(self):
        self.assertEqual(
            self.api.filter_name_list(
                ['1.in', '1.out', '2.in', '.DS_Store'], spj=False
            ),
            ['1.in', '1.out'],
        )
        ...
        self.assertEqual(
            self.api.filter_name_list(['1.in', '1.out', '2.in'], spj=True),
            ['1.in', '2.in'],
        )
    ...
    def test_upload_spj_test_case_zip(self):
        with open(self.make_test_case_zip(), 'rb') as f:
            resp = self.client.post(
                self.url, data={'spj': 'true', 'file': f}, format='multipart'
            )
            self.assertSuccess(resp)
            data = resp.data['data']
            ...
            for item in data['info']:
                ...
                with open(
                    os.path.join(test_case_dir, name), 'r', encoding='utf-8'
                ) as f:
    ...
    def test_upload_test_case_zip(self):
        with open(self.make_test_case_zip(), 'rb') as f:
            resp = self.client.post(
                self.url, data={'spj': 'false', 'file': f}, format='multipart'
            )
            self.assertSuccess(resp)
            data = resp.data['data']
...
class ContestProblemAdminTest(APITestCase):
    def setUp(self):
        ...
        self.create_admin()
        self.contest = self.client.post(
            self.reverse('contest_admin_api'), data=DEFAULT_CONTEST_DATA
        ).data['data']
...
class ContestProblemTest(ProblemCreateTestBase):
    ...
    def test_admin_get_one_contest_problem(self):
        ...
        resp = self.client.get(
            '{}?contest_id={}&problem_id={}'.format(self.url, contest_id, problem_id)
        )
        self.assertSuccess(resp)
    ...
    def test_regular_user_get_not_started_contest_problem(self):
        ...
        self.assertDictEqual(
            resp.data, {'error': 'error', 'data': 'Contest has not started yet.'}
        )
