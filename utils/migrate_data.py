...
def set_problem_display_id_prefix():
    while True:
        print(
            "Please input a prefix which will be used in all the imported problem's displayID"
        )
        print(
...
def get_test_case_score(test_case_id):
    ...
    for test_case in info['test_cases'].values():
        if test_case.__contains__('stripped_output_md5'):
            ...
            test_case['stripped_output_md5'] = get_stripped_output_md5(
                test_case_id, test_case['output_name']
            )
    ...
    if get_input_result():
        for data in users.values():
            if not email_regex.match(data['email']):
                print(
                    '%s will not be created due to invalid email: %s'
    ...
    if get_input_result():
        ...
        for data in problems:
            ...
            if Problem.objects.filter(_id=data['_id']).exists():
                ...
                continue
            try:
                creator_id = User.objects.filter(
                    username=users[data['created_by']]['username']
