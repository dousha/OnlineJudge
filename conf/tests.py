...
class JudgeServerHeartbeatTest(APITestCase):
    def setUp(self):
        ...
        self.headers = {
            'HTTP_X_JUDGE_SERVER_TOKEN': self.hashed_token,
    ...
    def test_update_heartbeat(self):
        ...
        self.assertSuccess(resp)
        self.assertEqual(
            JudgeServer.objects.get(hostname=self.data['hostname']).judger_version,
            data['judger_version'],
        )
...
class JudgeServerAPITest(APITestCase):
    ...
    def test_disabled_judge_server(self):
        resp = self.client.put(
            self.url, data={'is_disabled': True, 'id': self.server.id}
        )
        self.assertSuccess(resp)
