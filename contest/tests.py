...
class ContestAPITest(APITestCase):
    ...
    def test_regular_user_validate_contest_password(self):
        ...
        resp = self.client.post(
            url, {'contest_id': self.contest.id, 'password': 'error_password'}
        )
        self.assertDictEqual(
            resp.data, {'error': 'error', 'data': 'Wrong password or password expired'}
        )
...
class ContestAnnouncementAdminAPITest(APITestCase):
    ...
    def test_get_contest_announcements(self):
        ...
        response = self.client.get(
            self.url + '?contest_id=' + str(self.data['contest_id'])
        )
        self.assertSuccess(response)
...
class ContestRankAPITest(APITestCase):
    def setUp(self):
        user = self.create_admin()
        self.acm_contest = Contest.objects.create(
            created_by=user, **DEFAULT_CONTEST_DATA
        )
        self.create_user('test', 'test123')
